...
const TaskType DisplayTask = 1;
const TaskType TouchSensorTask = 2;

extern void TASKNAME( ActionTask )( void );
extern void TASKNAME( DisplayTask )( void );
extern void TASKNAME( TouchSensorTask )( void );

static __STK_UNIT _stack_ActionTask[__TCOUNT_STK_UNIT(512)];
static __STK_UNIT _stack_DisplayTask[__TCOUNT_STK_UNIT(512)];
static __STK_UNIT _stack_TouchSensorTask[__TCOUNT_STK_UNIT(512)];

const Priority tinib_inipri[TNUM_TASK] = { TPRI_MINTASK + 1, TPRI_MINTASK + 3, TPRI_MINTASK + 2, };
const Priority tinib_exepri[TNUM_TASK] = { TPRI_MINTASK + 1, TPRI_MINTASK + 3, TPRI_MINTASK + 2, };
const UINT8 tinib_maxact[TNUM_TASK] = { (1) - 1, (1) - 1, (1) - 1, };
const AppModeType tinib_autoact[TNUM_TASK] = { 0x00000000, 0x00000000, 0x00000000, };
const FP tinib_task[TNUM_TASK] = { TASKNAME( ActionTask ), TASKNAME( DisplayTask ), TASKNAME( TouchSensorTask ), };
const __STK_UNIT tinib_stk[TNUM_TASK] = { (__STK_UNIT)_stack_ActionTask, (__STK_UNIT)_stack_DisplayTask, (__STK_UNIT)_stack_TouchSensorTask, };
...
const AlarmType OSEK_Alarm_task_ts1 = 0;
const AlarmType OSEK_Alarm_task_ts2 = 1;
const AlarmType OSEK_Alarm_task_ts3 = 2;

DeclareTask(ActionTask);
static void _activate_alarm_OSEK_Alarm_task_ts1( void );
static void _activate_alarm_OSEK_Alarm_task_ts1( void )
{ (void)ActivateTask( ActionTask ); }

DeclareTask(DisplayTask);
static void _activate_alarm_OSEK_Alarm_task_ts2( void );
static void _activate_alarm_OSEK_Alarm_task_ts2( void )
{ (void)ActivateTask( DisplayTask ); }

DeclareTask(TouchSensorTask);
static void _activate_alarm_OSEK_Alarm_task_ts3( void );
static void _activate_alarm_OSEK_Alarm_task_ts3( void )
{ (void)ActivateTask( TouchSensorTask ); }

const CounterType alminib_cntid[TNUM_ALARM] = { 0, 0, 0, };
const FP alminib_cback[TNUM_ALARM] = { _activate_alarm_OSEK_Alarm_task_ts1, _activate_alarm_OSEK_Alarm_task_ts2, _activate_alarm_OSEK_Alarm_task_ts3, };
...
const TickType alminib_cycle[TNUM_ALARM] = { 4, 500, 20, };
